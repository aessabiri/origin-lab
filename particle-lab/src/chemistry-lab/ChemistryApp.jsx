...
const ChemistryApp = () => {
  ...
  return (
    <div className="flex flex-col w-full h-full bg-slate-900 text-white font-inter overflow-hidden relative">
      ...
      {/* Toast Notification */}
      <div 
        className={`absolute bottom-4 left-1/2 -translate-x-1/2 bg-amber-500 text-white px-6 py-2 rounded-full shadow-lg transition-all duration-300 pointer-events-none ${message ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      >
        {message}
      </div>
    </div>
